...
var stripe =
{
    ...
    setupCard: function(done)
    {
        if (window.setupIntentClientSecret) // Multishipping checkout & admin
            ...
            var clientSecret = stripe.stripePaymentForm.getSetupIntentClientSecret();
        ...
        stripe.stripeJs.confirmCardSetup(clientSecret, options).then(function(result)
        {
            ...
            if (stripe.stripePaymentForm)
                stripe.stripePaymentForm.refreshSetupIntent();
            else
    ...
    {
        if (stripe.stripePaymentForm)
            return stripe.stripePaymentForm.useSetupIntents();
